...
int aes_cbc_encrypt( aes_context *ctx, uint8 *in, uint32 inlen, uint8 **output, uint32 *outlen, uint8 ivec[16])
{
	int n = 0;
	uint8 *iv = ivec;
    uint8 *out = NULL;
...
int aes_cbc_decrypt( aes_context *ctx, uint8 *in, uint32 inlen, uint8 **output, uint32 *outlen, uint8 ivec[16])
{
	int n = 0;
	uint8 *iv = ivec;
    uint8 *out = NULL;
    ...
    append_len = out[-1] ;
    if (append_len > 16 || append_len < 1) {
        free(out - encrypt_len);
        return -1;
    }
    out[-append_len] = 0;
    *outlen = encrypt_len - append_len;
    *output = out - encrypt_len;
    return 0;
}
